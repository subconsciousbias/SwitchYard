...
start (the entrypoint does) or by hand:
    ...
    python -m switchyard.gen_litellm config/plans.yaml /tmp/litellm.generated.yaml
"""
...
def main() -> int:
    ...
    dst = sys.argv[2] if len(sys.argv) > 2 else "/tmp/litellm.generated.yaml"
    cfg = build(src)
    header = (
        "# GENERATED by `python -m switchyard.gen_litellm` — do not edit.\n"
        "# Edit config/plans.yaml instead; apply via `scripts/reload.sh`\n"
        "# (gateway hot-swap + portal refresh). POST /admin/reload on its\n"
        "# own only refreshes the portal board. The gateway regenerates\n"
        "# this file at container start, so a hand-run copy belongs in /tmp.\n"
    )
